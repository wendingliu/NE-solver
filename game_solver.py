...
class Game():
    ...
    def U(self, U):
        ...
        # self.nA = [|A0|,...,|An-1|]
        self.nA = list(U.shape[:-1])  # the number of available actions for each player on A
    ...
    def parse(self, z):
        ...
        '''
        last_index = np.cumsum(self.nA)  # the indexes for each player's last action if we ravel A 
        p_raw = z[ : np.sum(self.nA)]    # p_raw is (len=nA) list for startegy profile 
        v = z[np.sum(self.nA) : ]        # v is (len=n) list for value profile in NE              
        # rearange p_raw to use a nested list p to represent each player's strategy profile
        p=[] # p is the nested list of strategy profile
        for i in range(self.n):                    # loop for each player
            start = last_index[i-1] if i>=1 else 0 # index of first action of i if we ravel A
    ...
    def inner(self, z, S):
        ...
        p, v = self.parse(z) # p is the nested list of strategy profile, v is value profile
        ...
        eqs += [np.sum(pi) - 1 for pi in p]  # for each player, sum of (probabilities of) strategies is 1
            ...
            for ai in S[i]:        # loop for each action ai of i on Si
                ...
                for s in supports: # loop for all action profiles on S
                    if s[i] == ai: # if player i plays ai in action profile s
                        ...
                        u = self.U[s+(i,)] # utility of player i under action profile s
                        utility += prob*u  # update expected utility for i playing ai
                eqs.append(utility-vi)     # NE repuires all the actions on Si has same expected utility vi                  
    ...
    def outer(self, p, v, S):
        ...
        for pi in p: 
            ...
            if np.any(np.array(pi)<(-1e-6)):
                return False  # failure if some player plays an action with negative prob
        ...
        if self.A!=S: # if support profile is smaller than action profile
            for i in range(self.n): # loop for each player
                if self.A[i]!=S[i]: # if i's support is smaller than his action set
                    ...
                    Anew = S[:i] + [list(set(self.A[i]) - set(S[i]))] + S[i+1:] # nested list
        ...
        if outside: # if outside is not empty
            for i in range(self.n): # loop for each player
                if set(self.A[i])-set(S[i]): # if player i can take some action not on Si
                    vi = v[i]                # i's expected utility on S           
                    for ai in set(self.A[i])-set(S[i]): # loop for each action ai of i but not on Si
                        utility = 0        # expected utility if i plays ai
                        for s in outside:  # loop for all action profiles (tuple s) outside S
                            if s[i] == ai: # if player i plays ai in action profile s
                                # probability of s happens conditional on i plays ai (ai not in Si)
                                # prob(a_{-i}) = product(pj(aj)),j!=i
                                prob = np.product([p[j][s[j]] for j in range(self.n) if j!=i]) 
                                u = self.U[s+(i,)]   # utility of i under action profile s
                                utility += prob*u    # update expected utility for i playing ai not in Si
                        # failure if i playing any ai not in Si can improve his expected utility
                        if utility-vi > 1e-6: return False                                         
    ...
    def feasibility(self, S, method='hybr'):
        ...
        """
        # 1.solve all the (n+sum(An)) equations for (n+sum(An)) unkowns
        try:
            ...
            sol = optimize.root(f, [0.5]*np.sum(self.nA) + [0]*self.n, method=method)  
            x = sol.x
            x[np.abs(x) < 1e-6] = 0.  # change very small float number returned by scipy solver to 0
            x[:np.sum(self.nA)][(1-1e-6)<x[:np.sum(self.nA)]] = 1. # treat prob very close to 1 as 1
            # if support is for mixed strategy profile but each player plays pure strategy
            if (np.any(np.array([len(Si) for Si in S])!=1) 
                and set(x[ : np.sum(self.nA)])=={0., 1.}):
    ...
    def size_profile(self):
        ...
        if self.n == 2:
            ...
            X = np.array([[x0, x1] for x0 in range(1, self.nA[0]+1) for x1 in range(1, self.nA[1]+1)])   
            balance = np.abs(np.diff(X).ravel())  # balance of support profile: abs(x0-x1)  
            ...
            balance = np.max(X, axis=1) - np.min(X, axis=1) # balance of support profile: max_{i,j}(xi - xj)
            size = np.sum(X, axis=1)                        # size of support profile
    ...
    def dominated(self, i, a, Ai, R):
        ...
        '''
        for ai in set(Ai)-{a,}: # loop for all i's actions except a
            for b in R: # loop for all others's action profile, b=[b0,.., bi-1, bi+1,...,bn]
                # if playing *ai* cannot increase i's utility compared to *a* when others play b
                if (self.U[tuple(b[:i]) + (a,) + tuple(b[i:]) + (i,)] 
                    >= self.U[tuple(b[:i]) + (ai,) + tuple(b[i:]) + (i,)]):
                    break   # a is not strictly conditionally dominated by ai
            else:
                return True # a is strcictly conditionally dominated by ai 
        else:
    ...
    def algorithm1(self, only_one, method='hybr'):
        ...
        for x in self.X:  # loop for all size profile x (X has been sorted)
            ...
            for S0 in [S0 for S0 in all_S0 if len(S0)==x0]: # loop for each S0 which has the size of x0
                ...
                A1new = [a1 for a1 in A1 if not self.dominated(1, a1, A1, [[a0] for a0 in S0])]
                # if there is *not* any a0 in S0 which is conditionally dominated, given A1new
                if not np.any([self.dominated(0, a0, S0, [[a1] for a1 in A1new]) for a0 in S0]):
                    ...
                    for S1 in [S1 for S1 in self.powerset(A1new) if len(S1)==x1]:
                        ...
                        if not np.any([self.dominated(0, a0, S0, [[a1] for a1 in S1]) for a0 in S0]): 
        ...
        if NE: 
            ...
            print('no NE, there are bugs!') # finite n person game must has at least one NE
    
    def algorithm2(self, only_one, method='hybr'):
        '''Find all NEs in n-player game if only_one == False else only one smaple NE
        only_one: only return one sample NE if True
        method: algorithm used in scipy solver
        '''
        ...
        for x in self.X:  # loop for all size profile x (X has been sorted)
            S = [[] for i in range(self.n)]  # uninstantiated support profile
            # domain profile (nested list), D[i] is the list of i's supports which have size of x[i] 
            D = [[Si for Si in S_candidates[i] if len(Si)==x[i]] for i in range(self.n)]
            if only_one==True: 
                try:  
                ...
                if result: NEs += result
        # return all NEs (delete all duplicates) if only_one=False
        if only_one==False: return NEs

    def rb(self, S, D, i, only_one, method, NE=[]):
        '''
        Use recursive-backtracking method to find NEs given support prfile S and domain profile D
        S: support profile
        ...
        if i == self.n: # all Si in S have been filled 
            ...
            for di in D[i]:     # loop for each candidate di for i's support Si
                ...
                Dnew = [[S[k]] for k in range(i+1)] + D[i+1:] # use [S[k]] to replace D[k] for k=0,1,...,i                
                Dnew = self.irsds(Dnew) # self.irsds(D) update D if D is valid else return False    
                if Dnew: # if domain profile D is reasonable                     
                    try: # call self.rb agian until return NE or False 
                        if only_one==True: # call self.rb recursively until finding the first NE
                            p, v = self.rb(S, D=Dnew, i=i+1, only_one=True, method=method, NE=[])
                            return p, v # stop the loop
                        else: # if only_one=False, call self.rb recursively to find all NEs
                            NE = self.rb(S, D=Dnew, i=i+1, only_one=False, method=method, NE=NE)                            
                    except TypeError:
                        pass      
        if only_one==False: return NE # if only_one=False, return all the NEs under the initial (S, D, i)
            
    def irsds(self, D):
        '''Iteratated removal of strcitly sominated strategies (and check if D is a valid domain profile)
        D: domain profile (nested list), D[i] is the list of i's supports
        '''
        ...
        while changed: # if D changed
            ...
            for i in range(self.n): # loop for all players
                ...
                for ai in set(itertools.chain.from_iterable(D[i])):    
                    ...
                    for j in range(len(Dothers)): #D(-i)[j] is all the actions player j can take in D
                    ...
                    if self.dominated(i, ai, self.A[i], R): # if ai is conditionally dominated, given R
                        D[i] = [Si for Si in D[i] if ai not in Si] # shrink D since i won't play ai
                        changed = True
                        if not D[i]: return False # D is invalid if D[i] is empty
